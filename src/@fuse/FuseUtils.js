...
import * as roles from "../app/fuse-configs/dynamic-menu/rolesConfig";
...
class FuseUtils {
  ...
  static userRole = () => {
    //retorna el tag del rol almacenado en local storage
    const tokenRole = window.localStorage.getItem("infoUser");
    var _userRole = "";

    if (tokenRole) {
      //const decode = atob(tokenRole);
      const jsonAccess = JSON.parse(tokenRole);
      _userRole = jsonAccess.roles;
    }
    return _userRole;
  };

  static createNavigationDiaco = () => {
    //return _userRole;
    let arry = this.userRole();

    const found = arry ? arry.find((e) => e === "AGENTE") : null;

    const modules =
      found === "AGENTE" ? roles.ROLE.DIACO_ADMIN : roles.ROLE.DIACO_USER;
